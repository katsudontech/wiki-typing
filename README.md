...
**1. 非同期処理によるローディング待機の隠蔽（Pre-fetching）**
現在、ゲーム開始時に「Wikipediaからの記事取得」と「Gemini APIによるテキスト生成」を直列で行っているため、プレイ開始までの待機時間が長いという課題があります。
これを解決するため、初回の生成単位を小さくして最速でゲームを開始させ、**ユーザーがタイピングを行っている裏（バックグラウンド）で次の文章を非同期に事前生成（Pre-fetch）しておく仕組み**を実装し、待ち時間を感じさせないシームレスなUXを実現します。

**2. API制限の緩和とスケーラビリティの向上**
現在はGemini APIの無料枠を利用して運用していますが、連続プレイ時などのレートリミット（制限）が今後の課題です。商用レベルの安定したレスポンスを担保するため、有料枠へのアップグレードおよび、コストとパフォーマンスのバランスを取ったAPIリクエストの最適化を検討しています。

**3. スコア・フィードバックシステムの拡充**
ゲームとしてのリテンション（継続率）を高めるため、スコアリング機能を強化します。具体的には、WPM（1分間あたりの入力文字数）や正確率の算出に加え、Supabaseを利用したスコア履歴の保存・ランキング機能、苦手なキーの分析フィードバックなどの実装を予定しています。
